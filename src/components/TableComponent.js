...
// save to local storage button component
const SaveBtn = ({ data }) => {
...
const TableComponent = () => {
  ...
  return (
    <>
      <div className="flex flex-col my-8 border border-gray-100 rounded">
        {cryptoData ? (
            ...
            <tbody>
              {cryptoData.map((data) => {
                return (
                  <tr
                    key={data.id}
                    className="text-center text-base border-b border-gray-100 hover:bg-gray-200 last:border-b-0 transition-all"
                  >
                    <td className="py-4 flex items-center uppercase">
                      {/* save to local storage (Saved page) */}
                      <SaveBtn data={data} />
      ...
      {/* bottom info */}
      <div className="flex flex-col lg:flex-row gap-y-4 lg:gap-y-0 items-center justify-between mb-4 capitalize h-[2rem]">
        <span>
          Data provided by{" "}
          <a
